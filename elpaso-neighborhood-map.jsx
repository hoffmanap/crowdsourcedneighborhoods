...
function downloadGeoJSON(submissions) {
  ...
  a.href = url; a.download = "elpaso_neighborhoods.geojson"; a.click();
...
export default function App() {
  ...
  const [phase, setPhase] = useState("select"); // select | draw | done
  const [mapReady, setMapReady] = useState(false);
  const [turfReady, setTurfReady] = useState(false);
  const [loadError, setLoadError] = useState(null);
  const [submitted, setSubmitted] = useState(false);
  const [viewMode, setViewMode] = useState("both"); // both | consensus | submissions | none
  const [sessionSubmitted, setSessionSubmitted] = useState(new Set());
  const [activeTab, setActiveTab] = useState("draw"); // draw | view
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("elpaso_v2", true);
        if (r?.value) setSubmissions(JSON.parse(r.value));
      } catch (_) {}
    })();
  }, []);

  // Load Leaflet + Leaflet.draw + Turf
  useEffect(() => {
    ...
    const loadAll = async () => {
      const css = (id, href) => {
        ...
        Object.assign(document.createElement("link"), { id, rel: "stylesheet", href, onload: null })
          |> (el => document.head.appendChild(el));
      };

      // inline css loader to avoid pipeline operator issues
      ...
      map.on(L.Draw.Event.DRAWSTOP, () => { activeDrawRef.current = null; });
  ...
  const renderLayers = useCallback((subs, mode) => {
    ...
    const byN = {};
    subs.forEach(s => { if (!byN[s.neighborhood]) byN[s.neighborhood] = []; byN[s.neighborhood].push(s.coords); });
    ...
    Object.entries(byN).forEach(([name, list], idx) => {
      ...
      if (showSubs) {
        list.forEach(coords => {
          L.polygon(coords, { color, weight: 1.5, fillColor: color, fillOpacity: 0.07, opacity: 0.45, dashArray: "5 5" })
      ...
      if (showCons && count >= MIN_CONSENSUS && window.turf) {
        ...
        if (c) {
          ...
          const addPoly = (rings) => {
            L.polygon(rings, { color, weight: 3.5, fillColor: color, fillOpacity: opacity, opacity: 1 })
              .addTo(conLayerRef.current)
              .bindTooltip(`${name} — consensus of ${count} submission${count > 1 ? "s" : ""}`, { sticky: true });
          };
  ...
  const startDrawing = () => {
    ...
    const draw = new L.Draw.Polygon(leafletMap.current, drawControlRef.current.options.draw.polygon);
  ...
  const submitDrawing = async () => {
    ...
    setSubmitted(true);
    try { await window.storage.set("elpaso_v2", JSON.stringify(updated), true); } catch (_) {}
    drawnItemsRef.current?.clearLayers();
    setPhase("select");
    setSelectedN("");
  ...
  // Derived stats
  const counts = submissions.reduce((a, s) => { a[s.neighborhood] = (a[s.neighborhood] || 0) + 1; return a; }, {});
  ...
  // Color index lookup (consistent across renders)
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: "#f5f0eb", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header */}
      <header style={{ background: "#1a1a2e", color: "#f5f0eb", padding: "15px 26px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "3px solid #e63946", flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: "#777", marginBottom: 3, fontFamily: "monospace" }}>Crowdsourced Geography · El Paso, TX</div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, letterSpacing: "-0.01em" }}>Draw Your Neighborhood</h1>
        </div>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0, overflow: "hidden" }}>
        ...
        {/* Sidebar */}
        <div style={{ width: 292, minWidth: 292, background: "#fff", borderRight: "1px solid #e0d8cf", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {activeTab === "draw" && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ padding: "13px 17px", borderBottom: "1px solid #f0ece6" }}>
                ...
                <select
                  value={selectedN}
                  onChange={e => { setSelectedN(e.target.value); setPhase("select"); drawnItemsRef.current?.clearLayers(); }}
                  disabled={phase === "draw"}
                  style={{ width: "100%", marginTop: 9, padding: "9px 10px", border: "2px solid " + (selectedN ? "#e63946" : "#ddd"), borderRadius: 4, fontSize: 13.5, background: "#fff", cursor: "pointer", fontFamily: "inherit", color: selectedN ? "#1a1a2e" : "#999" }}
                >
                  <option value="">— Select neighborhood —</option>
                  {available.map(n => <option key={n} value={n}>{n}</option>)}
                  {sessionSubmitted.size > 0 && (
                    <optgroup label="Already submitted this session">
                      {[...sessionSubmitted].map(n => <option key={n} value={n} disabled>{n} ✓</option>)}
                    </optgroup>
                  )}
                </select>
                {sessionSubmitted.size > 0 && (
              ...
              {/* Step 2 */}
              <div style={{ padding: "13px 17px", borderBottom: "1px solid #f0ece6" }}>
                <StepLabel n={2} active={!!(selectedN && phase === "select")} done={phase === "done"}>Draw the boundary</StepLabel>
                ...
                {phase === "draw" && (
                  <div style={{ background: "#fff8e1", border: "2px solid #ffd166", borderRadius: 4, padding: "10px 12px", fontSize: 12.5, color: "#7a5c00", lineHeight: 1.5 }}>
                    ...
                    <br />
                    <button onClick={cancelDrawing} style={{ marginTop: 7, background: "none", border: "none", color: "#e63946", cursor: "pointer", fontSize: 12, padding: 0, textDecoration: "underline", fontFamily: "inherit" }}>
                      Cancel
                    </button>
                  </div>
                )}
                {phase === "done" && (
                  <div style={{ fontSize: 12.5, color: "#2a9d8f", background: "#f0faf8", padding: "9px 12px", borderRadius: 4, border: "1px solid #2a9d8f" }}>
                    ...
                    <br />
                    <button onClick={() => { drawnItemsRef.current?.clearLayers(); setPhase("select"); }} style={{ marginTop: 5, background: "none", border: "none", color: "#aaa", cursor: "pointer", fontSize: 11, padding: 0, textDecoration: "underline", fontFamily: "inherit" }}>Redraw</button>
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: "13px 17px", borderBottom: "1px solid #f0ece6" }}>
                ...
                <div style={{ marginTop: 10 }}>
                  <Btn enabled={phase === "done" && !!selectedN} color="#e63946" onClick={submitDrawing}>Submit My Map →</Btn>
                </div>
                {submitted && (
              ...
              <div style={{ flex: 1, padding: "13px 17px", overflowY: "auto" }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.14em", color: "#ccc", marginBottom: 10, fontFamily: "monospace" }}>Most mapped</div>
                {top.length === 0
                  ...
                  : top.map(([name, count], i) => (
                    <div key={name} style={{ marginBottom: 7, display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {count >= MIN_CONSENSUS && <span title="Consensus ready" style={{ fontSize: 9, color: "#2a9d8f", fontWeight: 700 }}>●</span>}
                      <span style={{ fontSize: 11, fontWeight: 700, color: COLORS[i % COLORS.length], fontFamily: "monospace" }}>{count}</span>
                    </div>
                  ))
                }
                {top.length > 0 && <div style={{ marginTop: 10, fontSize: 11, color: "#ccc" }}><span style={{ color: "#2a9d8f" }}>●</span> = Turf.js consensus boundary active</div>}
              </div>
            </div>
          )}
          ...
          {activeTab === "view" && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              {/* Layer control */}
              <div style={{ padding: "13px 17px", borderBottom: "1px solid #f0ece6" }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.14em", color: "#ccc", marginBottom: 12, fontFamily: "monospace" }}>Map layers</div>
                {[
                  ...
                  <label key={val} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10, cursor: "pointer" }}>
                    <input type="radio" name="vm" value={val} checked={viewMode === val} onChange={() => setViewMode(val)}
              ...
              {/* GeoJSON export */}
              <div style={{ padding: "13px 17px", borderBottom: "1px solid #f0ece6" }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.14em", color: "#ccc", marginBottom: 12, fontFamily: "monospace" }}>Export</div>
                <Btn enabled={submissions.length > 0} color="#2a9d8f" onClick={handleExport}>⬇ Download GeoJSON</Btn>
                ...
                {exportMsg && <div style={{ marginTop: 8, fontSize: 12, color: "#2a9d8f" }}>✓ Saved as elpaso_neighborhoods.geojson</div>}
              </div>

              {/* Full neighborhood status */}
              <div style={{ flex: 1, padding: "13px 17px", overflowY: "auto" }}>
                <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.14em", color: "#ccc", marginBottom: 10, fontFamily: "monospace" }}>All neighborhoods</div>
                {NEIGHBORHOODS.map((name) => {
                  ...
                  return (
                    <div key={name} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, opacity: count === 0 ? 0.35 : 1 }}>
                      ...
                      <div style={{ flex: 1, fontSize: 12.5, color: "#333" }}>{name}</div>
                      {count > 0 && <span style={{ fontSize: 11, fontFamily: "monospace", color: "#bbb" }}>{count}</span>}
                      {hasC && <span style={{ fontSize: 9, background: "#2a9d8f", color: "#fff", borderRadius: 3, padding: "1px 5px", fontFamily: "monospace", letterSpacing: "0.05em" }}>consensus</span>}
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ padding: "10px 17px", borderTop: "1px solid #eee", fontSize: 10.5, color: "#ccc", lineHeight: 1.5, flexShrink: 0 }}>
            All submissions shared globally. Consensus via Turf.js polygon union.
          </div>
        </div>
            ...
            <div style={{ position: "absolute", bottom: 28, left: 14, background: "rgba(255,255,255,0.94)", border: "1px solid #eee", borderRadius: 6, padding: "10px 14px", zIndex: 999, boxShadow: "0 2px 8px rgba(0,0,0,0.1)", fontSize: 11.5 }}>
              {(viewMode === "both" || viewMode === "submissions") && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <svg width="24" height="10"><line x1="0" y1="5" x2="24" y2="5" stroke="#999" strokeWidth="1.5" strokeDasharray="5 4" /></svg>
                  <span style={{ color: "#555" }}>Individual submissions</span>
                </div>
              )}
              {(viewMode === "both" || viewMode === "consensus") && (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <svg width="24" height="10"><line x1="0" y1="5" x2="24" y2="5" stroke="#e63946" strokeWidth="3.5" /></svg>
...
function Stat({ label, value, color }) {
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <div style={{ fontSize: 10, color: "#777", textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 2 }}>{label}</div>
    </div>
  );
}
...
function StepLabel({ n, active, done, children }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 5 }}>
      <div style={{ width: 21, height: 21, borderRadius: "50%", background: done ? "#2a9d8f" : active ? "#e63946" : "#ddd", color: "#fff", fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.25s", fontFamily: "monospace" }}>
        {done ? "✓" : n}
      </div>
      <span style={{ fontSize: 12.5, fontWeight: 600, color: (active || done) ? "#1a1a2e" : "#aaa", textTransform: "uppercase", letterSpacing: "0.06em", transition: "color 0.25s" }}>
        {children}
      </span>
    </div>
  );
...
function Btn({ enabled, color, onClick, children }) {
  return (
    <button onClick={onClick} disabled={!enabled} style={{ width: "100%", padding: "10px 12px", background: enabled ? color : "#d5d5d5", color: "#fff", border: "none", borderRadius: 4, fontSize: 13.5, cursor: enabled ? "pointer" : "not-allowed", fontFamily: "inherit", fontWeight: 600, transition: "background 0.2s, opacity 0.2s", opacity: enabled ? 1 : 0.65 }}>
      {children}
    </button>
  );
}
